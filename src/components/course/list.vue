<template>
  <b-container>
    <b-button variant="primary" class="mb-3" :to="{ name: 'add_course' }">
      新規追加
    </b-button>
    <b-row>
        ...
        >
          ...
          <template v-slot:footer>
            <b-row>
              <b-col>
                <b-button
                  :to="{ name: 'edit_course', params: { id: item.id } }"
                  href="#"
                  variant="warning"
                  sm
                >
...
export default {
  ...
  methods: {
    loadList() {
      ...
      this.get_axios("api/admin_manager/course")
        .then(res => {
          const data = {};
    ...
    hidecourse(id) {
      if (confirm("非表示にします。よろしいですか？")) {
        this.delete_axios("api/admin_manager/course/" + id).then(res => {
          this.loadList();
        });
      }
    },
    showcourse(id) {
      if (confirm("再表示します。よろしいですか？")) {
        this.put_axios("api/admin_manager/course/" + id + '/restore').then(res => {
          this.loadList();
        });
      }
    }
